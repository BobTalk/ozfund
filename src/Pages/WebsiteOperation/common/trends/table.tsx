...
import {
  DeleteTrendsInterface,
  GetTrendsInterface,
  SwitchTrendsInterface,
} from "@/api";
...
const TableProcess = (props, ref) => {
  ...
  async function changeTopCb(value, index) {
    let { status, message: tipInfo } = await SwitchTrendsInterface({
      id: value.id,
    });
  ...
  function deleteCb(e, crt, index) {
    stop(e, async () => {
      let { status, message: tipInfo } = await DeleteTrendsInterface({
        id: crt.id,
      });
      message[status ? "success" : "error"](tipInfo);
      if (status) {
